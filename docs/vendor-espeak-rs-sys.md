...
## Process-Scoped eSpeak Runtime Constraint

The sidecar currently keeps `std::env::set_var("PIPER_ESPEAKNG_DATA_DIRECTORY", ...)` in `src/main.rs` by design.

Reviewed constraint:

- `piper-rs 0.1.9` depends on `espeak-rs 0.1.9` for phonemization.
- `espeak-rs 0.1.9` initializes eSpeak through a one-time `Lazy` and reads only `PIPER_ESPEAKNG_DATA_DIRECTORY` when selecting the runtime data directory.
- There is no non-environment API in the current upstream dependency stack to inject the runtime path directly.
- Replacing the environment bridge would require upstream changes or additional vendored dependency work, which the parity-alignment ADR does not authorize.

Repository decision:

- keep the environment bridge as an internal startup mechanism,
- keep `--espeak-data-dir` as the only host-facing runtime selection input,
- do not add any request-scoped or `LINGOPILOT_*` compatibility alias for this behavior.
